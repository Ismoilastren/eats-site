...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setStep((current) => Math.min(current + 1, statuses.length - 1)), 4000);
    return () => window.clearInterval(timer);
  }, []);

  const eta = useMemo(() => Math.max(4, 24 - step * 4), [step]);
      ...
      <main className="mx-auto max-w-5xl px-4 pb-20 pt-6 lg:px-8">
        ...
        {!order ? (
          <div className="rounded-[40px] bg-white p-12 text-center">
            ...
            <p className="mt-2 font-bold text-gray-500">Local demo orders are stored in this browser.</p>
          </div>
        ) : (
